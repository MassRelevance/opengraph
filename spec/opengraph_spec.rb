...
describe OpenGraph do
  ...
  describe '.fetch' do
    ...
    it 'should catch errors' do
      ...
      RestClient::Request.should_receive(:execute).with(:method => :get, :url => 'http://example.com', :timeout => nil, :open_timeout => nil).and_raise(SocketError)
      OpenGraph.fetch('http://example.com').should be_false
    end

    it 'should throw OpenGraph::TimeoutError if timeout is provided' do
      stub_request(:get, 'http://example.com').to_timeout

      expect { OpenGraph.fetch('http://example.com', 5) }.to raise_error(OpenGraph::TimeoutError)
    end

    it 'should catch RequestTimeout if timeout is not provided' do
      stub_request(:get, 'http://example.com').to_timeout
